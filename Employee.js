    ...
    return (
        ...
        <div style={{ color: 'black', textAlign: 'center', paddingTop: '50px' }}>
            ...
            <div className='mt-4 bg-white rounded p-3' style={{ width: '60%', backgroundColor: 'white', padding: "30px" }}>
                <Link to="/createemp" className='btn btn-success m-2'>Add +</Link>
                <Link to={`/team/${id}`} className='btn btn-info m-2'>Team Details</Link>
                    ...
                    <tbody>
                        {
                            employee.map((data, i) => (
                                <tr key={i}>
                                    ...
                                    <td>{( id != data.ID && (data.Role == "admin" || data.Role == "org")) ?
                                        (<p>Not Allowed</p>) : 
                                        <>
                                        <Link to={`http://localhost:3000/employee/updateemp/${data.ID}`} className='btn btn-primary m-2'>Update</Link>
