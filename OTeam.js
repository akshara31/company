...
const handleDelete = async (id) => {
    ...
    return (
        ...
        <div className='mt-4 bg-white rounded p-3' style={{ backgroundColor: 'white', padding: '30px' }}>
                ...
                <Link to={`/oemployee/${id}`} className='btn btn-info m-2'>Personal Details</Link>
                    ...
                    <tbody>
                        {
                            team.map((data, i) => (
                                ...
                                <td>{data.Skills}</td>
                                    <td>
                                        <Link to={`http://localhost:3000/oteam/updateteam/${data.Employee_ID}`} className='btn btn-primary m-2'>Update</Link> 
