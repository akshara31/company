...
const handleDelete = async (ID) => {
    ...
    return (
        ...
        <div className='mt-4 bg-white rounded p-3' style={{ backgroundColor: 'white', padding: '30px' }}>
                <Link to="/createteam" className='btn btn-success m-2'>Add +</Link>
                <Link to={`/employee/${id}`} className='btn btn-info m-2'>Personal Details</Link>
                    ...
                    <tbody>
                        {
                            team.map((data, i) => (
                                ...
                                <td>{data.Skills}</td>
                                    <td>{( id != data.Employee_ID && (data.Name == "Admin") || (data.Name == "Org")) ?
                                        (<p>Not Allowed</p>) : 
                                        <>
                                        <Link to={`http://localhost:3000/team/updateteam/${data.Employee_ID}`} className='btn btn-primary m-2'>Update</Link> 
                                        <button className='btn btn-danger ms-2' onClick={ e => handleDelete(data.Employee_ID)}>Delete</button>
                                        </>
                                        }
                                    </td>
                                </tr>
